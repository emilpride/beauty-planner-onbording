...
export default function PhotoUploadBodyStep() {
  ...
  const handleCameraOpen = () => {
    alert('Camera capture is not implemented yet on web. Use Upload for now.')
  }
        ...
        <div className="p-3 space-y-3">
          <div className="flex items-center gap-3">
            <div className="relative h-16 w-16 rounded-lg overflow-hidden border border-gray-200 bg-transparent flex-shrink-0">
              <Image src={`/images/on_boarding_images/body_${genderStr}.png`} alt="Body reference" fill className="object-cover" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-sm font-semibold text-text-primary mb-1">Tips:</div>
              <ul className="list-disc pl-5 text-sm text-text-secondary space-y-1">
                <li>Natural light, full body in frame</li>
                <li>Stand straight with a neutral background</li>
                <li>Wear fitted clothing if possible</li>
              </ul>
            </div>
          </div>
          ...
          <div className="relative h-36 border-2 border-dashed border-purple-300 rounded-xl flex items-center justify-center transition-colors duration-200 p-2">
            {imageUrl ? (
              ...
              <button onClick={toggleSkip} className="w-full h-full flex flex-col items-center justify-center bg-gray-50 rounded-xl hover:bg-gray-100">
                <p className="font-semibold text-text-secondary">Skipped</p>
                <p className="text-xs text-gray-400 mt-1">Tap to undo</p>
              </button>
            ) : (
          ...
          <p className="text-[11px] text-gray-500 mt-2">Optional. If skipped, your plan will still be personalized based on previous answers.</p>
        </div>
      </div>
    </OnboardingStep>
  )
