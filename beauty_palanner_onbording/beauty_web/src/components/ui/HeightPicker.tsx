...
type HeightPickerProps = {
  ...
  gender?: 0 | 1 | 2 // 0 = unknown, 1 = male, 2 = female (for silhouette accent)
...
export default function HeightPicker({ value = 170, gender = 0, onConfirm, onCancel }: HeightPickerProps) {
  ...
  const getCharacterImage = () => {
    // Use onboarding silhouettes stored in public/images/on_boarding_images
    if (gender === 2) return '/images/on_boarding_images/bmi_female_2.png'
    if (gender === 1) return '/images/on_boarding_images/bmi_male_2.png'
    // Fallback to female for unknown to match onboarding preference (can be adjusted)
    return '/images/on_boarding_images/bmi_female_2.png'
  }
  ...
  return (
    <div className="w-full h-full bg-background flex flex-col select-none touch-none">
      ...
      <header className="flex items-center justify-between p-4 border-b border-border-subtle bg-surface/80 backdrop-blur-sm z-10">
        ...
        <button onClick={() => onConfirm(height)} className="text-sm text-accent font-bold transition-transform active:scale-95">Done</button>
      </header>

      {/* Stage */}
      <div ref={stageRef} className="relative flex-1 min-h-0 overflow-hidden bg-surface" onPointerDown={onPointerDown}>
        {/* Ruler Marks */}
        ...
        <div className="absolute left-0 right-0 z-[2] pointer-events-none" style={{ bottom: lineBottom }}>
          <div className="h-0.5 bg-gradient-to-r from-accent to-purple-500" />
...
const Ruler = ({ stageH, cmToFeetInches }: { stageH: number, cmToFeetInches: (cm: number) => { feet: number, inches: number, label: string } }) => {
  const marks = useMemo(() => {
    ...
    const numMarks = RULER_RANGE / 5 + 1
