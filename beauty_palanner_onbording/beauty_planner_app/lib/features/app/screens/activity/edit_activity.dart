...
class EditActivity extends StatelessWidget {
  const EditActivity({super.key, required this.activityModel});
  ...
  Widget build(BuildContext context) {
    ...
    activityModel.notifyBeforeActive.value =
        (activityModel.selectedNotifyBeforeFrequency!.isNotEmpty &&
            activityModel.selectedNotifyBeforeUnit!.isNotEmpty);

    activityModel.endBeforeActive.value =
        activityModel.endBeforeType == 'date'
            ? activityModel.selectedEndBeforeDate != null
            : activityModel.endBeforeUnit != null &&
                activityModel.endBeforeUnit!.isNotEmpty;
    return Scaffold(
      appBar: const BMAppbar(
        title: 'Edit Activity',
        // onBackPressed:
        //     () => Get.to(() => ActivityInfo(activity: activityModel)),
      ),
            ...
            return GestureDetector(
              ...
              child: Column(
                children: [
                  ...
                  _buildSaveButton(
                    isEnabled:
                        activityModel.type == 'regular'
                            ? ((activityModel.frequency == 'daily'
                                    ? true // Daily is always valid now
                                    : activityModel.frequency == 'weekly'
                                    ? activityModel.selectedDays.isNotEmpty
                                    : activityModel
                                        .selectedMonthDays
                                        .isNotEmpty) &&
                                (!activityModel.endBeforeActive.value ||
    ...
    return SizedBox(
      ...
      child: Opacity(
        ...
        child: ElevatedButton(
          onPressed: () {
            if (isEnabled) {
              activityModel.lastModifiedAt = DateTime.now();
              if (activityModel.type == 'regular') {
                userController.updateUserActivity(activityModel);
                ActivityController.instance.onActivityUpdated(activityModel);
                Loaders.customToast(message: 'Activity updated successfully');
              } else {
                activityModel.type = 'one_time';
                userController.updateUserActivity(activityModel);
                ActivityController.instance.onActivityUpdated(activityModel);
                Loaders.customToast(message: 'Activity updated successfully');
              }
