...
class PickAssistant extends StatelessWidget {
  const PickAssistant({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: BMAppbar(
        ...
        onBackPressed: () {
          Get.back();
          controller.assistant.value = controller.user.value.assistant;
        },
      ),
                  ...
                  child: Column(
                    children: [
                          ...
                          child: Row(
                            ...
                            children: [
                              _assistantOption(
                                ...
                                'Max',
                                isSelected: controller.assistant.value == 1,
                              ),
                              _assistantOption(
                                context,
                                AppImages.assistantEllie,
                                'Ellie',
                                isSelected: controller.assistant.value == 2,
                              ),
                            ],
                          ),
                        ),
                      ...
                      SizedBox(
                        ...
                        child: Opacity(
                          opacity:
                              controller.assistant.value != 0
                                  ? 1.0
                                  : 0.6, // Adjust opacity based on condition
                          child: ElevatedButton(
                            onPressed: () {
                              if (controller.assistant.value != 0) {
                                controller.user.update((user) {
                                  user?.assistant = controller.assistant.value;
                                });
                                UserRepository.instance.updateSingleField({
                                  'Assistant': controller.assistant.value,
                                });
                                Get.back();
                              }
                            },
  ...
  Widget _assistantOption(
    ...
    return GestureDetector(
      onTap: () {
        ...
        controller.assistant.value = label == 'Max' ? 1 : 2;
